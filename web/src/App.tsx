...
const viewerSteps = [
  ["Install the CLI", "Download a release binary or run the local installer from the repository."],
  ["Generate .depsaber/report.json", "Run depsaber report with your baseline and optional online checks."],
  ["Load local report", "Use the file picker. The browser reads the JSON locally and does not upload source code."],
  ["Review the delta", "Focus on new, existing, resolved, severity, ecosystem, evidence, and remediation."],
];
...
? Action        Delta scan`}</pre>
      ...
      <section className="pagesBand" id="pages">
        <div>
          <p className="eyebrow">Hosted viewer</p>
          <h2>Use your own report.</h2>
        </div>
        <p>
          Open the public viewer, load the JSON generated by DepSaber, and inspect the same risk summary
          your terminal or CI would use. The bundled sample is only there so the interface is not blank.
        </p>
        <div className="viewerStepGrid" aria-label="Report viewer workflow">
          {viewerSteps.map(([title, copy]) => (
            <article className="viewerStep" key={title}>
              <h3>{title}</h3>
              <p>{copy}</p>
            </article>
          ))}
        ...
        <code>depsaber report . --out .depsaber/report.json --online --baseline .depsaber/baseline.json</code>
      </section>
    </main>
  );
}
